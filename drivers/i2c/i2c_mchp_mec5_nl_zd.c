...
#define DT_DRV_COMPAT microchip_mec5_i2c_nl_zd
...
struct i2c_mec5_nl_data {
	...
	uint8_t xfr_flags;
	uint32_t cm_cmd;
#if 0 /* TODO remove. Will be replaced by DMA driver struct dma_block_config                       \
	 dma_blks[] */
 ...
 * Message maximum length is HW limit I2C_MEC5_NL_MAX_XFR_LEN.
 * Each message must not have a NULL data pointer or zero length.
 */
 ...
 * Implementation 1:
 * If state is closed
 *   1: I2C_NL START0=1, STARTN=0, STOP=1, wrLen=1, rdLen=len(msg)
 *      struct dma_block_config 1: src=&target read address, len=1, dir is
 * MEM_TO_PERIPH 2: I2C_NL START0=0, STARTN=0, STOP=1, wrLen=1, rdLen=len(msg)
 *      struct dma_block_config 2: dest=msgbuf, len(msg), dir is PERIPH_TO_MEM
 * else if state is WR
 *   1: I2C_NL START0=0, STARTN=1, STOP=1m, wrLen=1, rdLen=len(msg)
 *      struct dma_block_config 1: src=&target read address, len=1, dir is
 * MEM_TO_PERIPH 2: I2C_NL START0=0, STARTN=0, STOP=1, wrLen=0, rdLen=len(msg)
 *      struct dma_block_config 2: dest=msgbuf, len(msg), dir is PERIPH_TO_MEM
 * else if state is RD then error since we can't do two reads
 * The above depends on DMA driver reconfiguring DMA channel for next part of
 * transfer and making the channel callback. Channel callback will unpause
 * I2C-NL to process #2. Can we instruct DMA driver to not callback on second
...
static int do_i2c_rd(const struct device *dev, struct i2c_msg *m)
{
	...
	struct i2c_mec5_nl_data *const data = dev->data;
	struct mec_i2c_smb_ctx *hwctx = &data->ctx;
	struct dma_config *dcfg = &data->dma_cfg_cm;
	struct dma_block_config *dblks = &data->dma_blks[0];
	...
	/* TODO is it worth placing these structures contiguous and creating a helper
	 * functio to clear them all in one call?
	 */
	/* struct dma_config is ~32 bytes, full of bit-fields. memset should be faster
	 */
	memset(dcfg, 0, sizeof(struct dma_config));
	/* we need two struct dma_block_config's. Each is ~28 bytes */
	memset(dblks, 0, 2u * sizeof(struct dma_block_config));

	/* ensure we issue I2C STOP after every read otherwise this I2C-NL will not be
	 * happy */
#ifdef I2C_MEC5_NL_USE_INTERRUPTS
	...
	dblks[0].source_address = (uint32_t)data->xfrbuf;
	dblks[0].dest_address = (uint32_t)&i2c_regs->CM_TXB;
	dblks[0].block_size = 1u;
	dblks[0].dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;

	dblks[1].source_address = (uint32_t)&i2c_regs->CM_RXB;
	dblks[1].dest_address = (uint32_t)m->buf;
	dblks[1].block_size = m->len;
	dblks[1].source_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
	...
	dcfg->block_count = 1u;
	dcfg->head_block = &dblks[0];

	ret = dma_config(devcfg->dma_dev_cm, devcfg->cm_dma_chan, dcfg);
	if (ret) {
	 ...
	 * Do we want DMA callback for data read phase?
	 */
	dcfg->channel_direction = PERIPHERAL_TO_MEMORY;
	dcfg->head_block = &dblks[1];
	 ...
	 * in memory. When we run using debugger breakpoint after i2c_transfer the
	 * data pattern matches. Is the EC running at 96MHz so much faster than I2C at
	 * 400KHz and DMA at 48MHz that we return from i2c_transfer and do memory
	 * compare before DMA has transferred the last byte to memory. I know that
	 * I2C-NL does read-ahead but in this case it issues a STOP. I don't expect
	 * read-ahead to affect the timing in this case. To prove this add delay.
	 */
#ifdef I2C_MEC5_NL_USE_INTERRUPTS
	 ...
	 * the DMA channel has finished reading the last byte of data from
	 * I2C-NL.CM_RXB register and writing it to memory. We need to spin on the DMA
	 * channel waiting for it to terminate before returning from the function.
	 * Tested and works.
	 */
	do {
		dma_status = 0;
...
/* Configure and start hardware for a complete I2C write transaction.
 * I2C Write: START or RPT-START | transmit i2c-write-addr | check ACK |
 *            transmit N data bytes each with ACK check | optional STOP.
 * Generate START if bus state is IDLE (not open).
 * NOTE: HW requires a START for read to write.
 * HW requires target address to be the first byte in the buffer sent by DMA
 * to I2C-NL CM Transmit buffer register. This driver implements a transmit
 * buffer whose size is specified by device tree. The actual driver buffer
 * is at least 4 bytes larger than the DT size to leave room for start address
 * prepended to the message and optional repeated start address post-pended to
 * the message.
 *
 * If no (Rpt)Start required
 *    dma block 0: m->buf, m->len
 * Else
 *   if len(m) <= DT TX buffer size
 *     xfrlen = m->len + 1
 *     data->xfrbuf[0] = target write address
 *     memcpy(&data->xfrbuf[1], m->buf, m->len)
 *     dma block 0: data->xfrbuf, xfrlen
 *   else
 *     xfrlen = m->len + 1
 *     dma_block0_ptr = data->xfrbuf
 *     dma_block0_len = devcfg->cm_tx_buf_max_sz + 1
 *     dma_block1_ptr = m->buf[devcfg->cm_tx_buf_max_sz]
 *     dma_block1_len = m->len - devcfg->cm_tx_buf_max_sz
 *     data->xfrbuf[0] = target write address
 *     memcpy(&data->xbrbuf[1], m->buf, dma_block0_len)
 *     Make use of DMA driver ISR to load dma_block1 when dma_block0 is
 * finished.
 */
static int do_i2c_wr(const struct device *dev, struct i2c_msg *m)
{
	...
	if (mflags & I2C_MSG_RESTART) {
		ntx++; /* add one for target address */
		cm_flags |= MEC_I2C_NL_FLAG_START | MEC_I2C_NL_FLAG_FLUSH_BUF;
		data->xfrbuf[0] = fmt_addr(data->i2c_addr, I2C_NL_DIR_WR);

		if (m->len <= devcfg->cm_tx_buf_max_sz) {
			memcpy(&data->xfrbuf[1], m->buf, m->len);
			dblks[0].source_address = (uint32_t)&data->xfrbuf[0];
			dblks[0].dest_address = (uint32_t)&i2c_regs->CM_TXB;
			dblks[0].block_size = ntx;
			dblks[0].dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
		} else {
			memcpy(&data->xfrbuf[1], m->buf, devcfg->cm_tx_buf_max_sz);
			dblks[0].source_address = (uint32_t)&data->xfrbuf[0];
			dblks[0].dest_address = (uint32_t)&i2c_regs->CM_TXB;
			dblks[0].block_size = devcfg->cm_tx_buf_max_sz + 1u;
			dblks[0].dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
			dblks[0].next_block = &dblks[1];

			dblks[1].source_address = (uint32_t)m->buf[devcfg->cm_tx_buf_max_sz];
			dblks[1].dest_address = (uint32_t)&i2c_regs->CM_TXB;
			dblks[1].block_size = m->len - devcfg->cm_tx_buf_max_sz;
			dblks[1].dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;

			dcfg->block_count = 2u;
		}
	} else {
		dblks[0].source_address = (uint32_t)m->buf;
		dblks[0].dest_address = (uint32_t)&i2c_regs->CM_TXB;
		dblks[0].block_size = m->len;
		dblks[0].dest_addr_adj = DMA_ADDR_ADJ_NO_CHANGE;
	}
	...
	/* I2C-NL transmit: TX DMA finishes moving bytes from memory to I2C-NL.CM_TXB
	 * buffer register before I2C-NL finishes transmitting the last byte and
	 * checking (n)ACK. We deduce I2C-NL sets CM_DONE after the (n)ACK since it
	 * has a status bit for NAK from target and will stop the transfer if a NAK is
	 * received. Another timing issue: race condition with this polling code and
	 * HW. If we see CM_DONE==0 and before we read HW status again I2C-NL does
	 * STOP then I2C-NL HW will clear CM_DONE and set IDLE. This is strange
	 * because sometimes we see both CM_DONE and IDLE and we just hung here due to
	 * IDLE only: i2c_status = 0x2200_0081. An interrupt from another subsystem
	 * (kernel timer, etc.) could preempt reading I2C-NL HW allowing time for IDLE
	 * to occur. CM_DONE is supposed to be R/W1C (latched) so why do we miss
	 * CM_DONE? Because CM_CMD = 0x0002_0003. It is not done, wrCnt = 2. BUG is
	 * previous msg was I2C_MSG_WRITE | I2C_MSG_STOP, current msg is I2C_MSG_WRITE
...
/* i2c_addr is the HW captured 8-bit from the bus: b[7:1]=7-bit I2C target
 * address and bit[0] = R/nW direction bit.
 */
 ...
 * In both cases Host generated START and transmitted target address to this
 * target. I2C-NL rdCnt = len(driver TM RX buf) +  2. TM DMA is dev2mem from
 * I2C-NL to driver TM RX buf. len = rdCnt.
 * 1. I2C Write
 *    I2C-NL and TM DMA RX mode moved target write address and data bytes to
 * driver TM RX buf. Host wrote n < rdCnt bytes: TM_DONE=1 when STOP detected.
 * rdCnt decremented by n. PROC=0, RUN=0. Host wrote n == rdCnt bytes and issued
 * STOP. Same as above except rdCnt == 0. Host wrote n > rdCnt bytes. After
 * rdCnt bytes, I2C-NL NAK's any future bytes and sets TM_NAKR status causing
 * TM_DONE=1 Host wrote n <= rdCnt bytes where the last byte was target address
 * for RPT-START. TM_DONE=1, rdCnt decremented by n. PROC=0, RUN=1
 *
 * 2. I2C Read
 ...
 *      rdCnt decremented by 1, PROC=0, RUN=1. I2C-NL is in pause state with
 * clock stretched. We need to get application buffer via buf_read_req callback
...
static void handle_tm(const struct device *dev, uint32_t tm_cmd)
{
	...
	if (events & BIT(I2C_NL_KEV_TM_PAUSE_POS)) {
		...
		if (i2c_addr & BIT(0)) { /* Read? */
			...
			tm_rd(dev);
			return;
		} else { /* Host wrote data. Termination condition: STOP or recv buffer
			    filled */
			...
			mec_hal_dma_chan_ien(devcfg->tm_dma_chan,
					     (BIT(MEC_DMA_CHAN_STS_DONE_POS) |
					      BIT(MEC_DMA_CHAN_STS_HFC_TERM_POS)),
					     1u);
		}
	}
...
/* TODO */
/* I2C Target-Mode DMA channel callback */
static void i2c_mec5_nl_tm_dma_cb(const struct device *dma_dev, void *user_data, uint32_t channel,
				  int status)
{
